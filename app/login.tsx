...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const { token, user } = await apiLogin(syntheticEmail, password);
      haptic.medium();
    ...
    >
      <View style={[styles.content, { paddingTop: insets.top + spacing.xxl, paddingBottom: insets.bottom + spacing.lg }]}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: c.text }]}>Вход</Text>
        ...
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: c.textSecondary }]}>Нет аккаунта? </Text>
          <Pressable
            onPress={() => {
              haptic.light();
            ...
            disabled={loading}
          >
            <Text style={[styles.footerLink, { color: c.tint }]}>Регистрация</Text>
          </Pressable>
        </View>
      </View>
    </KeyboardAvoidingView>
